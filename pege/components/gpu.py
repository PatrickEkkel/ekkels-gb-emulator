...
class TileRow:
    ...
    def create_grid_row(self):
        for x in range(7, -1, -1):
            self.row.append(Screen.RED)
...
class GPU:
    ...
    def write_vram(self, vram_address, tile):
        offset = 0
        for i in tile:
    ...
    def render_nintento_logo(self):
        ...
        for x in range(MMU.BACKGROUND_MAP_START, 0x9930,16):
            ...
            self.write_vram(x, tile)

        nintendo_tiles = {
            0x8000: [0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00],
            #0x8000: [0x3C,0x00,0x42,0x00,0xB9,0x00,0xA5,0x00,0xB9,0x00,0xA5,0x00,0x42,0x00,0x3C,0x00],
        ...
        tilemap_ram  = MMU.TILERAM_START
        vram_start   = MMU.VRAM_START
        bg_map_start = MMU.BACKGROUND_MAP_START
        bg_map_end   = MMU.BACKGROUND_MAP_END
        self.LY = 0
        gridy_id = 0
        gridx_id = 0
        pointer = 0
        ...
        for scy in range(0, self._screen.height):
            ...
            for scx in range(0, width):
                current_offset = offset + scx
                tile_id = self._mmu.read(current_offset)
                tile_address = tilemap_ram + (tile_id * 16)
